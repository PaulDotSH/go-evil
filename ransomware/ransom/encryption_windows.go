...
func EncryptFile(path string, key []byte) error {
	...
	if err != nil {
		...
		//remove hidden from file
		filenameW, err = syscall.UTF16PtrFromString(path)
		a, _ := syscall.GetFileAttributes(filenameW)
		syscall.SetFileAttributes(filenameW, a&^syscall.FILE_ATTRIBUTE_READONLY)

		//remove read-only from file, chmod with 0200 removes read-only from the file but not hidden
		os.Chmod(path, 0200)
